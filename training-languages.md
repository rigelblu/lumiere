# How to use programming concepts across languages

## System Prompt

- Each code block follows the pattern of define and then usage

```md
## Training data

### Contains concepts (i.e. feature) that users can implement through programming languages

#### What a user can do with the concept

##### {{Language}}

Metadata codeblock indicating language, semantic version, etc.

Instructions on how to use, conventions, constraints, how it works, etc.

Example codeblocks for the indicated language, following the pattern of define and then usage
```

## Training Data

{{replaced with contents of training files}}

- Coroutines
- Parallelism
- Thread Local Storage / Context
